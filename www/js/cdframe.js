...
var CDpages = {
    ...
    goto: function (page_name, page_para) {
        if (CDapp.children().length == 0) {
            ...
            $('.app>div:last').removeClass("on");//当前页面z-index减小
            var page = CDpages.pages[page_name];
            var page_html = page.page(page_para);
            CDapp.append(page_html);
            CDpages._init_page(page_name);
    ...
    _init_page: function (page_name) {//初始化页面样式
        ...
        var $page = $('#' + page_name);
        //如果页面高度小于屏幕高，设置页面高度
        if (parseInt($page.css("height")) < window.screen.height - 20) {
            $page.css("height", window.screen.height - 20 + "px");
        }
        $page.addClass("on");
        // 如果页面需要从右侧滑入
        if ($page.hasClass('slideIn')) {
            $page.removeClass('slideIn');
            $page.addClass('slideInRight');
        }
    }
}
//框架主文件
